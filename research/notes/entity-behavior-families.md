...
remaining work is to trace the callback's internal helper calls while holding
the object at a known camera position, then repeat with both pair variants.
...
The cloud, paper, wind, bump, UFO-effect, platform, pickup, and puzzle-letter
families have dispatch boundaries and sprite/resource correlations. Platforms
are the only one with a durable implementation-side interaction hypothesis:
the current engine treats active platform footprints as one-way floor. That is
a compatibility behavior, not proof of the original executable's response.
...
1. one paired normal enemy family per world (`01/02`, `05/06`, `09/0A`,
   `15/16`, `19/1A`);
2. platforms (`3D`-`40`) because collision and motion are high-value;
3. pickups and puzzle letters (`6F`-`72`, `79`-`7F`) because removal/global
   interaction is user-visible;
4. cloud, paper, wind, bump, and UFO effects;
5. cross-world leaf and cloud resource checks.

Each new trace should update the corresponding seven dimension statuses in the
JSON matrix and add a durable reference to the interpretation note. Do not
promote a dimension to `confirmed` solely because a callback or sprite slot is
known.
