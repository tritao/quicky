...
The fresh main-repository callback traces answer natural jump initiation, the ascent-to-fall transition, and a subsequent landing/contact transition. Fresh side-probe traces answer call order and movement through a clear corridor. The low-Y ceiling trace is explicitly controlled and therefore does not assign floor/ceiling polarity. The moving-platform evidence is archival from the same executable but a different tracer worktree; a current-main capture stopped at the ARE declaration timeout. Those unresolved runtime points keep the implementation gate closed.
