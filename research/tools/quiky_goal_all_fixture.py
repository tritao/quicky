...
def run(args: argparse.Namespace) -> int:
    ...
    next_offset = next((r.target_offset for r in references
                        if r.target_offset - target_offset >= 44), len(are_data))
    if next_offset - target_offset < 44:
    ...
    overwritten_refs = [r.reference for r in references
                        if target_offset <= r.target_offset < target_offset + 44]
    ...
    for letter_index in range(7):
        _put_u16be(moved, cursor, 0x79 + letter_index)
    ...
    for reference, index, _entity in letters:
        ...
        if source_entity.record_offset < target_offset or \
           source_entity.record_offset >= target_offset + 44:
