#!/usr/bin/env python3
"""Run the controlled 50B1 high-score finalization probe."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--seed-aux", type=int, default=13)
    parser.add_argument("--seed-progression", type=int, default=14)
...
def run(args: argparse.Namespace) -> int:
    ...
    script_path = repo_root / "research/automation/quiky_highscore_jump_probe.lua"
    ...
    try:
        ...
        prefix = "TRACE_CONFIG = " + lua_literal({
            ...
            "seed_aux": args.seed_aux,
            "seed_progression": args.seed_progression,
        }) + "\n"
        api.request(
            "POST", "/api/v1/script/load?name=high-score-jump-probe",
        ...
        while time.monotonic() < deadline:
            ...
            if output.get("awaiting_startup_replay") and not replayed:
                ...
                replayed = True
            if output.get("awaiting_name_input"):
                now = time.monotonic()
                if name_stage == 0:
                    api.post("/api/v1/input/type", {"text": "AAAAAAA"})
                    name_stage = 1
                    name_stage_time = now
                elif name_stage == 1 and now - name_stage_time >= 1.5:
                    # 01D7:055F treats scan-code 1C (Enter) as the name
                    # terminator.  Space is an ordinary name character.
                    api.post("/api/v1/input/type", {"text": "\n"})
                    name_stage = 2
            if isinstance(output.get("high_score_jump_trace"), dict):
                trace = output["high_score_jump_trace"]
                break
            time.sleep(0.05)
        if trace is None:
            raise TraceError("timed out waiting for high_score_jump_trace")
...
def main() -> int:
    ...
    try:
        ...
        print(f"highscore_jump_probe: {error}", flush=True)
        return 2
